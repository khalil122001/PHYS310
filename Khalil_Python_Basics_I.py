...
for i in range(-size, size+1): # iterating from -5 to 5 and skipping 0 and 1 to avoid repetition of the single *
    if (i!=0) and (i!=1) :
        ...
        for j in range(0, size-i):
            print(' ', end="") #print an empty space each iteration and then reversing it once the positive numbers are reached
        for j in range(0, 2*i-1):
            print("*", end="") #printing 1,3,5,7,9 * each time
...
while i*i <= input_Value:
    ...
    if i*i == input_Value:
        ...
        break 
    if (i >  (input_Value/2 + 1 )): # since the square root will always be less than or equal to half the original value 
